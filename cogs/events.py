...
class Events(commands.Cog):
        ...
        async def on_member_join(self, member):
            ...
            if channel:
                embed = discord.Embed(description='Welcome to our guild!', colour=random.choice(self.bot.colour_list))
        ...
        async def on_member_remove(self, member):
            ...
            channel = discord.utils.get(member.guild.text_channels, name='recording')
            if channel:
                embed = discord.Embed(description='Goodbye from all of us..', colour=random.choice(self.bot.colour_list))
                embed.set_thumbnail(url=member.avatar_url)
                embed.set_author(name=member.name, icon_url=member.avatar_url)
                embed.set_footer(text=member.guild, icon_url=member.guild.icon_url)
                embed.timestamp = datetime.datetime.utcnow()
